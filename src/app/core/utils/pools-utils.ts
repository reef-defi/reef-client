...
export const getBasketPoolsAndCoins = (
  ...
  const poolNames = Object.keys(basket).filter((isPool: string) =>
    isPool.includes(' ')
  );
  const coinNames = Object.keys(basket).filter(
    (isPool: string) => !isPool.includes(' ')
  );
  const extractedPoolData = allPools.filter((poolData: IPoolsMetadata) =>
    poolNames.includes(poolData.Symbol)
  );
  ...
  const coins = Object.keys(allCoins)
    ...
    .map((coinName) => ({
      coinName,
      allocation: +basket[coinName],
      address: allCoins[coinName],
    }));
...
const getName = (addresses: string[], allTokens: any): string => {
  ...
  const n = Object.keys(allTokens).filter((name: string) =>
    addresses.includes(allTokens[name])
  );
  if (n.length === 0) {
    const word = addresses.reduce(
      (memo, curr) =>
        `${memo}${memo.length === 0 ? '' : '/'}${curr.slice(0, 5)}`,
      ''
    );
    return `${prefix}${word}`;
  }
...
export const basketNameGenerator = () => {
  ...
  return word
    .split(/ /g)
    .map((x: string) => `${x.substring(0, 1).toUpperCase()}${x.substring(1)}`)
    .join(' ');
};
...
export const getBasketPoolNames = (
  baskets: IBasket[],
  pools: IPoolsMetadata[],
  tokens: any
) => {
  let newB = baskets.map((basket) => {
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      if (pools[i].ExchangeName === 'Uniswap-v2') {
        const pair = basket.UniswapPools.pools.find(
          (poolPair) =>
            (poolPair[0].toLocaleLowerCase() === addr1 &&
              poolPair[1].toLocaleLowerCase() === addr2) ||
            (poolPair[0].toLocaleLowerCase() === addr2 &&
      ...
      if (pools[i].ExchangeName === 'Balancer') {
        const address = basket.BalancerPools.pools.find(
      ...
      if (pools[i].ExchangeName === 'Mooniswap') {
        const address = basket.MooniswapPools.pools.find(
          (addr) => addr === pools[i].ExchangeAddress
        );
        if (address) {
  ...
  newB = newB.map((basket) => {
    ...
    return {
      ...
      Tokens: {
        ...basket.Tokens,
        pools:
          temp.length === basket.Tokens.pools.length
            ? [...temp]
            : [...basket.Tokens.pools],
      },
    };
