...
export class VaultsPage implements OnInit {
  ...
  ngOnInit(): void {
    this.vaults$.pipe(take(1)).subscribe((vaults: IVault) => {
      this.vaults = vaults;
      this.currentVaults = Object.keys(this.vaults)
        .slice(0, 3)
        .reduce(
          (memo, curr) => ({
            ...memo,
  ...
  editVaultAlloc([name, percentage]): void {
    ...
    for (const key in this.currentVaults) {
      if (key !== name) {
        this.currentVaults[key].percentage = Math.round(
          this.currentVaults[key].percentage * multiplier
        );
      }
    }
    this.setChart(this.chartMonthsSpan);
  ...
  setChart(subtractMonths: number = 1): void {
    ...
    const payload = Object.keys(this.currentVaults).reduce(
      (memo, curr) => ({
        ...memo,
    ...
    this.apiService
      .getHistoricRoi(payload, this.chartMonthsSpan)
      .pipe(take(1))
      .subscribe((data) => {
        this.vaultRoiChart = this.chartsService.composeHighChart(
          this.extractRoi(data)
        );
      });
  }
  ...
  private getVaultPercentage(vaults: IVault): IVault {
    ...
    return Object.keys(vaults).reduce(
      (memo, curr) => ({
        ...memo,
        [curr]: {
          ...vaults[curr],
